...
export default function MilestoneView({ userId, roleId }: { userId: string; roleId: string }) {
  ...
  return <section style={{ display: "grid", gap: 16, fontFamily: li.font }}>
    <div style={panel}><p style={{ margin: 0, color: li.green, fontSize: 12, fontWeight: 700, textTransform: "uppercase" }}>Your path</p><h2 style={{ margin: "4px 0", color: li.textPrimary }}>Progress toward {path.role.name}</h2><p style={{ margin: "4px 0 0", fontSize: 13, color: li.textSecondary }}>Based on 2,000+ profiles who made this transition</p><p style={{ color: li.textSecondary }}>{path.skillGaps.filter((g) => g.status === "missing").length ? `${path.skillGaps.filter((g) => g.status === "missing").length} skill${path.skillGaps.filter((g) => g.status === "missing").length === 1 ? "" : "s"} separate you from this role today.` : "Your profile already covers the role's core skill set."} {done.length} of {path.milestones.length} actions complete · {path.readinessScore}% role readiness</p><div style={{ height: 10, background: li.cardBorder, borderRadius: 8, overflow: "hidden" }}><div style={{ height: "100%", width: `${percent}%`, background: li.green, transition: "width .25s ease" }} /></div><p style={{ marginBottom: 0, fontSize: 12, color: li.textHint }}>{checkedAt ? `Last check-in ${new Date(checkedAt).toLocaleDateString()}` : "Check off actions as you complete them."}</p></div>
    <div style={panel}><h3 style={{ marginTop: 0 }}>Timeline</h3>{[0, 1, 2, 3].map((pi) => { const items = active.filter((m) => phaseFor(m.order) === pi); return <div key={pi}><div style={{ display: "flex", gap: 12, padding: "14px 0" }}><div style={{ width: 48, height: 48, borderRadius: 8, background: li.blueLight, color: li.blue, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 700, flexShrink: 0 }}>Q{pi + 1}</div><div style={{ minWidth: 0, flex: 1 }}><div style={{ fontSize: 16, fontWeight: 600, color: li.textPrimary }}>{phaseLabel(pi)}</div>{items.length ? items.map((m) => <MilestoneItem key={m.order} milestone={m} checked={false} target={targetFor(path.generatedAt, m.order)} onToggle={() => toggle(m.order)} />) : <p style={{ margin: "8px 0 0", fontSize: 13, color: li.textHint }}>Goals coming soon</p>}</div></div>{pi < 3 && <hr style={{ border: "none", borderTop: `1px solid ${li.cardBorder}`, margin: 0 }} />}</div>; })}</div>
